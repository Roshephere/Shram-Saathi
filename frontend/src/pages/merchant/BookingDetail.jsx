    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border p-6">
        ...
        {booking.special_notes && (
          <div className="mt-4">
            <h2 className="font-semibold text-gray-900 flex items-center gap-2 mb-2"><FileText className="h-4 w-4" /> Customer Notes</h2>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              {serviceRequest.budget_min && serviceRequest.budget_max && (
                <p className="text-sm text-gray-500 mt-1">
                  Budget: NPR {Number(serviceRequest.budget_min).toLocaleString()} - {Number(serviceRequest.budget_max).toLocaleString()}
                </p>
              )}
            </div>
          </div>
      ...
      <div className="flex justify-end gap-3">
        ...
        {booking.status === 'in_progress' && (
          <button onClick={() => doAction('complete')}
            className="flex items-center gap-1.5 px-5 py-2.5 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 shadow-sm">
